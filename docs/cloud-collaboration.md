...
Agent Taskboard can run as a small shared Cloudflare deployment for two trusted collaborators:
...
| --- | --- |
| Worker | `board` |
| D1 database | `agent-taskboard-db` |
| R2 bucket | `agent-taskboard-attachments` |
...
```bash
npx wrangler d1 create agent-taskboard-db
npx wrangler r2 bucket create agent-taskboard-attachments
```
