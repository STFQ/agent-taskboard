...
impl UpdateDialog {
    fn prompt(_app: &AppHandle, version: &str) -> Option<Self> {
        let message = format!("发现 Agent Taskboard {version}。是否现在下载、安装并重启？");
        ...
        let dialog = run_on_main(move |mtm| {
            ...
            alert.setMessageText(&NSString::from_str("Agent Taskboard 更新"));
...
impl UpdateDialog {
    fn prompt(app: &AppHandle, version: &str) -> Option<Self> {
        app.dialog()
            .message(format!(
                "发现 Agent Taskboard {version}。是否现在下载、安装并重启？"
            ))
            .title("Agent Taskboard 更新")
...
impl LauncherState {
    fn new(
        ...
        Self {
            ...
            data_directory,
            log_path: log_directory.join("agent-taskboard-launcher.log"),
        }
    }
}
...
fn resolve_legacy_skill_conflict(
    ...
    let proceed = app
        ...
        .title("Agent Taskboard Skill 冲突")
...
fn watch_launcher_output<R: std::io::Read + Send + 'static>(
    ...
    thread::spawn(move || {
        for line in BufReader::new(reader).lines().map_while(Result::ok) {
            ...
            } else if !is_stderr && line.contains("Agent Taskboard listening") {
                update_snapshot(&app, &state, |snapshot| {
                    if state.generation.load(Ordering::SeqCst) == generation
                        && snapshot.child_pid == Some(pid)
                    {
...
fn main() {
    let app = tauri::Builder::default()
        ...
        .setup(|app| {
            ...
            #[cfg(target_os = "macos")]
            let data_directory = home_directory.join("Library/Application Support/Agent Taskboard");
            #[cfg(target_os = "macos")]
            let log_directory = home_directory.join("Library/Logs/Agent Taskboard");
            #[cfg(target_os = "windows")]
                ...
                .on_menu_event(move |app, event| match event.id().as_ref() {
                    ...
                    "open-taskboard-web" => {
                        ...
                        tauri::async_runtime::spawn_blocking(move || {
                            if let Err(error) = open_taskboard_in_browser(&state) {
                                ...
                                show_error_dialog(&app, "Agent Taskboard 网页打开失败", &error);
                            }
                        });
                    }
                    "restart-codex" => {
                    ...
                    "autostart" => {
                        ...
                        if let Some(error) = operation_error.or(sync_error) {
                            show_error_dialog(app, "Agent Taskboard 自启动设置失败", &error);
                        }
                    }
                    "quit" => {
                        let Some(state) = app.try_state::<Arc<LauncherState>>() else {
