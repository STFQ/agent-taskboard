...
if (target === windowsTarget && process.platform !== "win32") {
  throw new Error("Agent Taskboard for Windows must be prepared on Windows");
}
if (target === linuxTarget && process.platform !== "linux") {
  throw new Error("Agent Taskboard for Linux must be prepared on Linux");
}
if (target !== windowsTarget && target !== linuxTarget && process.platform !== "darwin") {
  throw new Error("Agent Taskboard for macOS must be prepared on macOS");
}
...
async function prepareLinuxNodeRuntime() {
  ...
  const targetPath = path.join(
    binariesDirectory,
    `agent-taskboard-node-${linuxTarget}`,
  );
  await mkdir(binariesDirectory, { recursive: true });
  await rm(targetPath, { force: true });
...
async function copyApplicationResources() {
  ...
  if (target === windowsTarget) {
    const taskctlWrapper = [
      "@echo off",
      "setlocal",
      "set \"CODEX_TASKBOARD_DATA_DIR=%APPDATA%\\Agent Taskboard\"",
...
export CODEX_TASKBOARD_DATA_DIR="\${XDG_DATA_HOME:-$HOME/.local/share}/Agent Taskboard"
export CODEX_TASKBOARD_RUNTIME_FILE="$CODEX_TASKBOARD_DATA_DIR/launcher-runtime.json"
exec "$RESOURCE_DIR/../../bin/agent-taskboard-node" "$RESOURCE_DIR/app/cli/taskctl.mjs" "$@"
`;
...
export CODEX_TASKBOARD_DATA_DIR="$HOME/Library/Application Support/Agent Taskboard"
export CODEX_TASKBOARD_RUNTIME_FILE="$CODEX_TASKBOARD_DATA_DIR/launcher-runtime.json"
