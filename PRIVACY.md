...
Agent Taskboard is a local-first application. Its desktop launcher runs the
...
On Windows, Agent Taskboard stores its database, attachments, launcher runtime
file, and independent Codex browser profile under:

`%APPDATA%\Agent Taskboard`

Launcher logs are stored under:

`%LOCALAPPDATA%\Agent Taskboard\Logs`
...
Agent Taskboard does not include advertising or a project-maintainer analytics
service.
